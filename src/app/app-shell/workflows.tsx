...
export default function WorkflowPanel({ role }: WorkflowPanelProps) {
  ...
  return (
    <section className="dashboard-panel" aria-labelledby="workflow-heading">
      <div className="panel-heading"><div><span className="eyebrow">OPERATIONS</span><h2 id="workflow-heading">Workflows</h2></div><span className="status-chip">Live API</span></div>
      {canCreate && <form className="auth-form" onSubmit={createWorkflow}>
      ...
      {loading ? <p>Loading workflows…</p> : workflows.length === 0 ? <p>No workflows are configured for this organization yet.</p> : <div>
        {workflows.map((workflow) => {
          const nextStatuses = canRequestLifecycleChange ? NEXT_STATUSES[workflow.status] : []
          return <div className="activity-row" key={workflow.id}>
            <div><strong>{workflow.name}</strong><span>{workflow.description || 'No description'}</span></div>
            <div><span>{workflow.status}</span>{nextStatuses.map((status) => <button key={status} type="button" className="header-link" disabled={updatingWorkflowId === workflow.id} onClick={() => void transitionWorkflow(workflow, status)}>{status}</button>)}</div>
          </div>
        })}
      </div>}
    </section>
